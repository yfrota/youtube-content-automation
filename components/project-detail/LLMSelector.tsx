...
export function LLMSelector({ projectId, stageKey, value, onChange, label }: LLMSelectorProps) {
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <select
        ...
        disabled={saving}
        className="h-9 w-fit min-w-[240px] rounded-lg border border-gray-200 bg-background px-2.5 text-xs text-foreground outline-none transition-colors duration-200 focus:border-accent disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700"
      >
        {LLM_PROVIDERS.map((p) => (
