...
`app/dashboard/DashboardContent.tsx` — filters/sort live in the URL (`?q=`, `?clientId=`, `?platform=`, `?tag=`, `?sort=`, `?order=`). "Todos os projetos" / "Por cliente" toggle (`?view=byClient`); grouping is client-side `Map`-building over the already-fetched array. Always sends `clientId=<filter>||"all"` explicitly to `GET /api/projects`. `clientIdFilter` (the `?clientId=` value) is shared between `ProjectsToolbar`'s client dropdown and `StatsRow`'s "Visão" selector via the same `handleClientChange` handler — picking a client in either one updates the URL, which re-filters both the project grid and the KPI cards (`StatsRow` re-fetches `GET /api/dashboard/stats?clientId=` for its "Vídeos indexados" KPI; the other 3 KPIs are computed straight from the already-filtered `projects` array).
