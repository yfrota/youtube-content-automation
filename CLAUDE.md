...
**Next 16 requires `useSearchParams()` behind a `<Suspense>` boundary or the production build fails** ("Missing Suspense boundary with useSearchParams"). Dev mode never surfaces this — only `npm run build` does. `app/(app)/projects/page.tsx` and `app/(app)/projects/new/page.tsx` are both thin Suspense wrappers for this reason. Apply the same pattern to any future page that adds `useSearchParams`.

**Dashboard vs. Projects split** (Opção C): `/dashboard` is a lean overview, `/projects` is the full filterable/groupable list. This split exists so the landing page stays fast to scan; anyone wanting the full catalog clicks through.

`app/dashboard/DashboardContent.tsx` — overview only, **not the full project list**. No `useSearchParams` (single filter, plain `useState`, so `app/dashboard/page.tsx` no longer needs a Suspense wrapper). "Visão geral" section reuses `StatsRow` unchanged. "Atividade recente" section shows only the 6 most recent projects (`RECENT_PROJECTS_LIMIT`), scoped to the same "Visão" client selector StatsRow uses, plus a "Ver todos os projetos →" link to plain `/projects` (no query string — lets that page's own groupBy default/localStorage resolution apply).

`components/projects/ProjectsView.tsx` (rendered by `app/(app)/projects/page.tsx`) — the full list, with a 2-column sidebar-folder layout. Filters/sort/grouping all live in the URL (`?q=`, `?clientId=`, `?platform=`, `?tag=`, `?sort=`, `?order=`, `?groupBy=platform|client|none`). **`platform` and `clientId` are two independent URL dimensions used uniformly regardless of which UI control set them** — when `groupBy=platform`, the sidebar folders set `platform` and the toolbar's cross-filter dropdown sets `clientId`; when `groupBy=client`, it's reversed; when `groupBy=none`, both come from the toolbar and there's no sidebar. This is why `ProjectsToolbar` grew `showClientFilter`/`showPlatformFilter` (both default `true`) — whichever dimension the active folder already controls gets hidden from the toolbar to avoid a redundant control. `groupBy`'s default resolves URL → `localStorage["halo-projects-groupby"]` (via `useSyncExternalStore`, same pattern as `StatsRow`'s KPI selection) → `"platform"`; changing the pills writes both. Content within the active folder subgroups by the *other* dimension (platform folder → subgroups by client; client folder → subgroups by platform; `groupBy=none` → flat grid). Folder counts come from a **separate unfiltered fetch** (`GET /api/projects?clientId=all`, no `q`/`tag`) so the sidebar taxonomy stays stable while the user types — the content grid itself uses a second, fully-filtered fetch. Delete/edit apply optimistic patches to both fetched lists rather than refetching either.
...
- `/analytics`, `/settings` top-level routes — still "Em construção" placeholders (`/projects` is fully built, see `components/projects/ProjectsView.tsx` above).
