...
- API routes: `GET /api/projects` (dashboard list), `POST /api/projects` (create), `GET /api/projects/[id]` (single project + its latest script/seo/thumbnail status, for the detail page), `PATCH /api/scripts/[id]` (status transitions, e.g. the "Aprovar roteiro" button → `kelly_review`), `POST /api/connectors/youtube/index`, `POST /api/agents/script-forge`. **None have auth/session checks** — each has a `// TODO(auth):` comment; this is accepted for now since they're server-only and there's no real tenant-membership model to check against yet. `POST /api/projects` and `POST /api/agents/script-forge` both accept `clientId` as optional, falling back server-side to `DEV_CLIENT_ID` — this is what lets client components call them without that env var ever reaching the browser. Keep this pattern for any new mutation route a client component needs to call directly.
- `app/dashboard/page.tsx` — fetches `GET /api/projects` client-side (skeleton while loading, distinct error state vs. empty state — don't conflate "fetch failed" with "no projects yet"). Clicking a card navigates to `/projects/[id]` (no more inline accordion — that UI moved to the detail page). "Novo projeto" navigates to `/projects/new` (a real form now, not a placeholder).
- `app/(app)/projects/new/page.tsx` — create-project form (title, platform fixed to YouTube, channel URL stored verbatim in `external_channel_id` — it's just descriptive metadata here, not validated as a real YouTube channel id). Posts to `POST /api/projects`, redirects to `/dashboard`.
- `app/(app)/projects/[id]/page.tsx` — the project detail page: a vertical pipeline stepper (`components/project-detail/PipelineStage.tsx` for the shell — numbered circle, connector line, active/complete/locked styling, optional collapse — plus `ScriptStage.tsx` for the real stage-1 UI and `LockedStageContent.tsx` for the locked-stage message). Stage 1 (Roteiro) is fully functional: paste a transcript → `POST /api/agents/script-forge` → review hook/chapters/content → "Aprovar roteiro" → `PATCH /api/scripts/[id]` to `kelly_review`, which unlocks stage 2. Stages 2-4 (SEO/Thumbnail/Checklist) are locked-by-default UI only — stage 2 shows a "Gerar SEO" button once unlocked, but it's a placeholder (toast, no backend yet); stages 3-4 can never actually unlock until SEO/Thumbnail get real approval flows, which is expected for now.
- `lib/dashboard/types.ts` — shared types for both the list shape (`Project`/`PipelineModule`) and the detail shape (`ProjectDetail`/`ScriptDetail`/`ScriptChapter`). **`checklist` has no backing table** — there's no dedicated checklist/publish-gate table in the schema, so both `GET /api/projects` and `GET /api/projects/[id]` map it to the project's own `projects.status` column (the closest real equivalent to an overall publish gate). If a real `checklist` concept gets its own table later, update both routes and this mapping note.
- `GET /api/projects` (and `/[id]`) read `DEV_CLIENT_ID` (env var) as a stand-in for the authenticated user's client — **temporary, remove when real auth lands**. The list route also filters to `external_video_id IS NULL`: rows with that set are catalog imports from `/api/connectors/youtube/index` (already-published existing videos), not content actually in production, and the dashboard is for the latter only.
...
- SEO Engine, Thumbnail Studio, Post-Production Assistant, Publish Checklist agents — none have a real generation flow yet. The detail page's stages 2-4 are deliberately locked-forever UI until these exist; that's by design, not a bug to fix blindly.
