...
Next.js 16 (App Router, Turbopack) + TypeScript + Tailwind + Supabase (Postgres + pgvector) + Anthropic Claude (agents) + OpenAI embeddings. No test framework is set up — verification is manual (`npm run build` / `tsc --noEmit` + curl against the API routes).
...
- Next migration should be `0003_*.sql` — don't renumber existing ones.
...
- `lib/rag/embeddings.ts` — OpenAI `text-embedding-3-small`, 1536 dims (matches the schema; don't change one without the other).
...
- `lib/agents/script-forge.ts` — Anthropic `claude-sonnet-4-6`, **forced tool-use** (`tool_choice: {type: "tool", name: "emit_script"}`) to get structured `{content, hook, chapters}` back instead of parsing free text. Pulls RAG context from `searchCatalog` before generating.
...
## Type gotcha (already hit this once — don't reintroduce it)
...
See `.env.example` — `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `YOUTUBE_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` are the ones currently used. `YOUTUBE_CLIENT_ID`/`SECRET` and `YOUTUBE_REDIRECT_URI` are placeholders for if/when OAuth gets added later — nothing reads them yet.
